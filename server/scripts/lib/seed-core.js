...
const users = [
  ["alice@example.com", "123456", "张三", "https://i.pravatar.cc/80?img=1", 2850],
  ["bob@example.com", "123456", "李四", "https://i.pravatar.cc/80?img=2", 1920],
  ["carol@example.com", "123456", "王五", "https://i.pravatar.cc/80?img=3", 4100],
];

const questions = [
  ["React 中 useState 和 useReducer 的区别是什么？", "什么时候应该使用 useReducer？", 1, 1245, 15, 1],
  ["如何优化 Next.js 应用的首屏加载速度？", "已尝试懒加载和代码分割。", 2, 892, 22, 1],
  ["TypeScript 中 interface 和 type 有什么区别？", "想了解二者的核心差异。", 3, 2103, 31, 0],
];
...
export const seedDatabase = async (conn) => {
  ...
  await conn.query("TRUNCATE TABLE votes; TRUNCATE TABLE follows; TRUNCATE TABLE comments; TRUNCATE TABLE answers; TRUNCATE TABLE question_tags; TRUNCATE TABLE questions; TRUNCATE TABLE article_tags; TRUNCATE TABLE articles; TRUNCATE TABLE tags; TRUNCATE TABLE search_history; TRUNCATE TABLE auth_tokens; TRUNCATE TABLE remote_jobs; TRUNCATE TABLE topics; TRUNCATE TABLE users;");
  await conn.query("SET FOREIGN_KEY_CHECKS=1");

  for (const [email, password, name, avatar, reputation] of users) {
    const passwordHash = await bcrypt.hash(password, 10);
    await conn.query("INSERT INTO users (email, password_hash, name, avatar, reputation) VALUES (?, ?, ?, ?, ?)", [email, passwordHash, name, avatar, reputation]);
  }
  ...
  await conn.query("INSERT INTO answers (question_id, author_id, content, votes, is_accepted) VALUES (1,3,'useState 适合简单状态，useReducer 适合复杂状态转移。',23,1),(2,1,'从关键渲染路径和缓存策略入手。',15,1)");
  ...
  await conn.query("INSERT INTO articles (title,excerpt,content,author_id,cover,views,likes,comments_count) VALUES ('深入理解 React Server Components','从渲染边界理解 RSC。','content',1,'https://images.unsplash.com/photo-1633356122544-f134324a6cee?w=800&h=400&fit=crop',3456,128,45),('TypeScript 类型体操实战','结合业务讲解高级类型。','content',2,'https://images.unsplash.com/photo-1516116216624-53e697fedbea?w=800&h=400&fit=crop',2890,95,32)");
